...
class Music(commands.Cog):
    ...
    async def cog_command_error(self, ctx: commands.Context, error: commands.CommandError):
        await ctx.send('Меня это пугает. Пошел нахуй: {}'.format(str(error)))
...
async def on_member_join(member):
    await member.send(embed=discord.Embed(title= '**Добро пожаловать на 11-SQUAD!**',description=f'Я - уникальный бот этого сервера, и для того, чтобы узнать список моих команд, пройдите в текстовый чат <#916330506419863594>.\n\n'
                                                                                                 f'Чтобы с самого начала проводить время на сервере комфортно, мы рекомендуем вам ознакомиться с чатом <#881620902037569616>.\n\n'
                                                                                                 f'Приятного общения!', colour=discord.Color.green()))

    if role := member.guild.get_role(905847749675724812):
...
async def on_voice_state_update(member, before, after):
    if after.channel != None:
        if after.channel.id == 906901115898048542:
...
bot.run('OTE1NjE4ODQzODA2NzkzODE4.YaeOfw.vG9z1761h_Cirbck1SHDvCado50')
